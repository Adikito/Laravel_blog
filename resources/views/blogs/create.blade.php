...
<div class="max-w-3xl mx-auto px-6">
    <div class="mb-10">
        <h1 class="text-4xl font-semibold text-white mb-3 tracking-tight">Create New Blog</h1>
        <p class="text-gray-400">Share your thoughts with the world</p>
    </div>

    <div class="border border-gray-800 rounded-lg p-8 bg-gray-900/50">
        <form method="POST" action="{{ route('blogs.store') }}" enctype="multipart/form-data" class="space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    >
                    <p class="mt-2 text-xs text-gray-500">Max 2MB. JPEG, PNG, JPG, GIF</p>
                    @error('image')
                        <p class="mt-2 text-sm text-gray-400">{{ $message }}</p>
                    @enderror
                </div>
